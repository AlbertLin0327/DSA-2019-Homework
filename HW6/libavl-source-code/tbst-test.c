...
static int
check_traverser (struct tbst_traverser *trav, int i, int n, const char *label)
{
  int okay = 1;
  int *cur, *prev, *next;

  prev = tbst_t_prev (trav);
  if ((i == 0 && prev != NULL)
                 ...
                 int insert[], int delete[], int n, int verbosity)
{
  ...
  /* Test creating a TBST and inserting into it. */
  ...
  /* Test TBST traversal during modifications. */
  for (i = 0; i < n; i++)
    {
      struct tbst_traverser x, y, z;
      int *deleted;
      ...
      if (tbst_t_find (&x, tree, &insert[i]) == NULL)
        {
      ...
      deleted = tbst_delete (tree, &delete[i]);
      ...
      if (tbst_t_insert (&z, tree, &delete[i]) == NULL)
        {
          if (verbosity >= 0)
  ...
  for (i = 0; i < n; i++)
    {
      ...
      {
        struct tbst_table *copy = tbst_copy (tree, NULL, NULL, NULL);
        if (copy == NULL)
          {
            if (verbosity >= 0)
        ...
        okay &= compare_trees (tree->tbst_root, copy->tbst_root);
        tbst_destroy (copy, NULL);
      }
    }
...
static int
test_bst_t_first (struct tbst_table *tree, int n)
{
  struct tbst_traverser trav;
  int *first;

  first = tbst_t_first (&trav, tree);
...
static int
test_bst_t_last (struct tbst_table *tree, int n)
{
  struct tbst_traverser trav;
  int *last;

  last = tbst_t_last (&trav, tree);
...
static int
test_bst_t_find (struct tbst_table *tree, int n)
{
  int i;
...
static int
test_bst_t_insert (struct tbst_table *tree, int n)
{
  int i;
  ...
  for (i = 0; i < n; i++)
    {
      ...
      iter = tbst_t_insert (&trav, tree, &i);
...
static int
test_bst_t_next (struct tbst_table *tree, int n)
{
  struct tbst_traverser trav;
  int i;
...
static int
test_bst_t_prev (struct tbst_table *tree, int n)
{
  struct tbst_traverser trav;
  int i;
...
static int
test_bst_copy (struct tbst_table *tree, int n)
{
  struct tbst_table *copy = tbst_copy (tree, NULL, NULL, NULL);
  int okay = compare_trees (tree->tbst_root, copy->tbst_root);
...
/* Tests the tree routines for proper handling of overflows.
   ...
   with height greater than |TBST_MAX_HEIGHT|.
   Uses |allocator| as the allocator for tree and node data.
               ...
               int order[], int n, int verbosity)
{
  ...
  typedef int test_func (struct tbst_table *, int n);
  ...
  for (i = test; i < test + sizeof test / sizeof *test; i++)
    {
      ...
      for (j = 0; j < n; j++)
        {
          void **p = tbst_probe (tree, &order[j]);
          if (p == NULL || *p != &order[j])
            {
